#--------------------------------------------------------------------------------------
#' Loading MN MDH HHBW source to toxval from toxval_source
#'
...
toxval.load.mn_mdh_hhbw <- function(toxval.db, source.db, log=FALSE, remove_null_dtxsid=TRUE){
  ...
  nlist = nlist[!nlist %in% c("casrn","name",
                              # Relationship field used for adding toxval_relationship table records
                              "relationship",
  ...
  #####################################################################
  #TODO Update to fit this source's case
  relationship = res %>%
    # Select needed columns
    dplyr::select(toxval_id, toxval_type, relationship) %>%
    # Define a "derived" toxicity value vs. DRSV toxval_type
    dplyr::mutate(
      derivative_type = dplyr::case_when(
        toxval_type %in% c("BMDL10", "NOAEL", "BMCL 0.25sd",
                           "BMDL05", "LOAEL", "BMDL5",
                           "BMDL (ADM 1SD)", "BMDL", "BMDL1SD",
                           "NOAEL (ADJ)", "UCDL10") ~ 1,
        TRUE ~ 0
      )
    ) %>%
    # Group by relationship tag
    dplyr::group_by(relationship, derivative_type) %>%
    # Collapse derived type vs. DRSV
    dplyr::mutate(dplyr::across(toxval_id, ~ toString(.))) %>%
    dplyr::ungroup() %>%
    dplyr::distinct() %>%
    dplyr::select(toxval_id, derivative_type, relationship) %>%
    # Pivot so 0 (DRSV) and 1 (toxicity value) toxval_id columns are created
    tidyr::pivot_wider(names_from = derivative_type,
                       values_from = toxval_id) %>%
    dplyr::rename(toxval_id_1 = `0`, toxval_id_2 = `1`) %>%
    # Set relationship
    dplyr::mutate(relationship = "derived from") %>%
    # Expand collapsed toxval_id columns
    tidyr::separate_longer_delim(toxval_id_1, delim = ", ") %>%
    tidyr::separate_longer_delim(toxval_id_2, delim = ", ") %>%
    # Remove those without a relationship
    tidyr::drop_na(toxval_id_1, toxval_id_2)

  # Insert relationships into toxval_relationship table
  ...
  res$source_url = "https://www.health.state.mn.us/communities/environment/risk/guidance/gw/table.html"
  res$details_text = paste(source,"Details")
