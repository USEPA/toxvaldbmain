---
title: "ToxValDB v9.7.0 Release Notes"
author: Taylor Wall
description: >
  The following are notes to accompany a release for ToxValDB.
resource_files:
- images/tovaldb_workflow.png
format: 
  html:
    toc: TRUE
    toc-depth: 3
...
![Overall dataflow for the construction of ToxValDB.](images/toxvaldb_workflow.png)
:::
...
```{r}
#| label: "tbl-new-sources-desc"
#| echo: FALSE
#| tbl-cap: !expr 'paste0("New Sources included in ToxValDB ", toxval_version)'

new_src_desc = readxl::read_xlsx(paste0(toxval.config()$datapath, 
                                       "/release_files/v9_7_0/new source tables_20250909.xlsx"))

new_src_rec_count = runQuery(paste0("SELECT source_table, count(*) as 'Record Count' ",
                                    "FROM toxval ",
                                    "WHERE source_table in ('", 
                                    paste0(new_src_desc$source_table, collapse = "', '"), 
                                    "') GROUP BY source"),
                             toxval.db)

new_src_desc = new_src_desc %>%
  dplyr::left_join(new_src_rec_count,
                   by = "source_table") %>%
  dplyr::select(-source_table) %>%
  dplyr::relocate(`Record Count`, .after=`Included ToxVal Type(s)`)

DT::datatable(new_src_desc, 
              options = list(
                scrollX = TRUE,
                columnDefs = list(list(className = 'dt-left',
                                       targets = 0:(length(new_src_desc))
                                       )
                                  )
                ),
              # rownames = FALSE,
...
src_version_date = lapply(seq_len(nrow(tbl_list)), function(i){
  ...
  return(
    data.frame(source = tbl_list$source[i],
               source_table = t_name,
              source_version_date = toString(sort(unique(t_version))))
    )
}) %>%
  dplyr::bind_rows()
...
-   Deduplication hierarchy was updated to account for new sources:
    -   "NIOSH" > "NIOSH IDLH"
    -   "Cal OEHHA" > "Cal OEHHA REL derivations"
-   The "study_group" field generation was updated to include the "study_duration_class" field.
-   The toxval_type field mapping for Cal OEHHA was updated from "Reference Exposure Level" to "REL".
-   For qc_fail flag assignment logic, we removed a case where "g/kg" would be flagged as "Ambiguous toxval_units". This affected records from ECHA IUCLID (2), ECOTOX (8), EFSA (4), HAWC Project (2), and IRIS (1). Due to time constraints, they could not be reincorporated into the main database but do remain in the qc_fail log files for reference.
-   For qc_fail flag assignment logic, we removed a case where "gerbils" were flagged as "species out of scope". This affected records from ECHA IUCLID (13) and ATSDR MRLs (1). Due to time constraints, they could not be reincorporated into the main database but do remain in the qc_fail log files for reference.

## New Fields

-   **Toxicological Effect Category** Toxicological effects were manually assigned to one of the following toxicological effect categories: body weight, cancer, clinical chemistry, clinical signs, developmental, enzyme activity, food and/or water consumption, gross pathology, hematology, mortality/survival, multiple, neurobehavior, neurotransmitter, none, nonneoplastic histopathology, organ weight, other, reproductive, and urinalysis. Categories were adopted from the 2018 WHO/IPCS guidance document, “Guidance document on evaluating and expressing uncertainty in hazard characterization” and were applied in a manner consistent with published approaches (https://doi.org/10.1080/10937404.2025.2552108) (note DOI may not be live until later in 2025). See Data Landscape Figures section, figure "Record Count by Study Type Toxicological effect Category" for a visualization of the categories by record count and study type.
