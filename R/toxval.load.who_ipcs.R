#--------------------------------------------------------------------------------------
#' Load the who_ipcs data  from toxval source db to toxval
#'
...
#--------------------------------------------------------------------------------------
toxval.load.who_ipcs <- function(toxval.db, source.db, log=FALSE, remove_null_dtxsid=TRUE){
  ...
  # Handle toxval_numeric ranges, select the minimum value
  res$toxval_numeric[grepl("-", res$toxval_numeric)] = res$toxval_numeric[grepl("-", res$toxval_numeric)] %>%
    sub('-.*', '', .)
  #####################################################################
  ...
  print(paste0("Dimensions of ref data: ", toString(dim(refs))))
