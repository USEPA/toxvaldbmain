...
#--------------------------------------------------------------------------------------
toxval.load.atsdr.pfas.2021 <- function(toxval.db, source.db, log=FALSE, remove_null_dtxsid=TRUE){
  ...
  res = res %>% dplyr::mutate(
    study_duration_value = gsub(".+\\-", "", study_duration_value)
  )
  ...
  # Dynamically remove non-ToxVal columns
