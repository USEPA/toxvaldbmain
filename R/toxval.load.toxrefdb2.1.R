...
#--------------------------------------------------------------------------------------
toxval.load.toxrefdb2.1 <- function(toxval.db,source.db, log=FALSE, remove_null_dtxsid=TRUE, do.init=FALSE){
  source = "ToxRefDB"
  ...
  if(do.init) {
    ...
    for (i in 1:nrow(temp)){
      row <- temp[i,]
      res[i,"key"] = digest::digest(paste0(row,collapse=""), serialize = FALSE)
