...
fix.study_group <- function(toxval.db, source=NULL, subsource=NULL, report.only=FALSE) {
  ...
  if(!report.only){
    # Reset to "-"
    runQuery(paste0("update toxval set study_group='-' where source in ('",source_string,"')", query_addition), toxval.db)
  }
  ...
  if(!report.only){
    query = paste0("update toxval set study_group=CONCAT(source,':',toxval_id,':',sex,':',generation,lifestage) ",
                   "where source in ('",source_string,"')'",
                   query_addition)
    runQuery(query,toxval.db)
  } else {
    query = paste0("SELECT toxval_id, CONCAT(source,':',toxval_id,':',sex,':',generation,lifestage) as study_group from toxval ",
                   "where source in ('",source_string,"')",
                   query_addition)
