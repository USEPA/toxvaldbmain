...
#' @export
#--------------------------------------------------------------------------------------
toxval.load.pprtv.cphea <- function(toxval.db, source.db, log=F){
  source <- "PPRTV (CPHEA)"
  source_table = "source_pprtv_cphea"
  runQuery("update source_chemical set source ='PPRTV (CPHEA)' where source='PPRTV CPHEA'",source.db)
  verbose = log
  #####################################################################
  ...
  #####################################################################
  res$subsource <- "EPA ORD CPHEA"
  name.list = names(res)
  name.list[is.element(name.list,"species")] = "species_original"
  names(res) = name.list
  if(is.element("species_original",names(res))) res[,"species_original"] <- tolower(res[,"species_original"])
  res[is.element(res$toxval_units,"?g/m^3"),"toxval_units"] = "ug/m3"
  res[is.element(res$toxval_units,"mg/m^3"),"toxval_units"] = "mg/m3"
  res[is.element(res$toxval_type,"Oral Slope Factor"),"toxval_type"] = "cancer slope factor"
  res[is.element(res$toxval_type,"Unit Risk Factor"),"toxval_type"] = "cancer unit risk"
  res[is.element(res$study_type,"cancer"),"study_type"] = "chronic"
  res$study_type = tolower(res$study_type)

  res$critical_effect = paste(res$assessment_type,"noncancer: ",res$endpoint,":",res$critical_effect," cancer:",res$tumor_site,res$cancer_site)
  res$long_ref = res$study_reference
  res$guideline = res$confidence
  ##########################################################
