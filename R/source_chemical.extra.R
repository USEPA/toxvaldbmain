...
source_chemical.extra <- function(toxval.db,
  ...
  # Pull curated DTXSID (rerun source_chemical.extra after pushing curated chemicals)
  dtxsid_list = runQuery(paste0("SELECT chemical_id, dtxsid FROM source_chemical WHERE chemical_id in ('",
                                paste0(res$chemical_id, collapse = "', '"), "')"),
                         source.db)
  # Add DTXSID values
  res = res %>%
    dplyr::left_join(dtxsid_list,
                     by = "chemical_id")
